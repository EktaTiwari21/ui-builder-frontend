...
/**
 * SignupFormClient client-side rendering component.
 * Validates user name, email, and minimum 6-character passwords.
 * Submits queries to secure signup API endpoints.
 */
export default function SignupFormClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        general: err instanceof Error ? err.message : "Failed to register. Please try again later.",
      });
    } finally {
      setIsLoading(false);
    }
        ...
        <Card className="border border-slate-200 bg-white shadow-xl shadow-slate-150 rounded-2xl overflow-hidden relative">
          ...
          <CardHeader className="pt-8 pb-4">
            ...
            <CardDescription className="text-center text-slate-500 text-sm font-medium">
              Start generating high-fidelity React components in seconds.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="pb-6 pt-2">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider" htmlFor="name-input">
                  Full Name
                </label>
          ...
          <CardFooter className="bg-slate-50/50 border-t border-slate-100 py-4 flex justify-center text-xs">
            <span className="text-slate-500 font-medium mr-1">Already have an account?</span>
